...
class UnsponsoredDates extends Component {
  constructor(props) {
    super(props);
    this.state = {
      showDateModal: false,
  ...
  confirmDate = async () => {
    ...
    const { dateOptions, matchId, firebase, match, uid } = this.props;
  ...
  render() {
    ...
    const unsponsoredDateOptionsArray = Object.keys(dateOptions)
      .filter(
        dateOptionId =>
          dateOptions[dateOptionId] &&
          (dateOptions[dateOptionId].eligibility === 'all' ||
            (!dateOptions[dateOptionId].eligibility &&
              dateOptions[dateOptionId].codeType === 'multiuse')),
      )
      .map(dateOptionId => ({ [dateOptionId]: dateOptions[dateOptionId] }));

    if (unsponsoredDateOptionsArray.length === 0) {
      return <div id="banner"></div>;
    }

    const unsponsoredDateOptions = Object.assign(
      ...unsponsoredDateOptionsArray,
    );

    const times = unsponsoredDateOptions[dateOptionId]?.times || [
      '11:00AM',
      '12:00PM',
      '1:00PM',
      '2:00PM',
    ...
    const dateOptionNames = Object.values(
      unsponsoredDateOptions,
    ).map(dateOptionValue =>
    ...
    if (dateInfo.confirmed) {
      ...
      const canCancel = timeScheduled - now > 0;

      return (
        <div sx={datesSx}>
          <div className="date-banner pink-background" id="banner">
            <div>
              Congrats, you've scheduled a {dateOptionDisplay} date at{' '}
          ...
          >
            <Modal.Header closeButton>
              <Modal.Title>
                {dateOptionDisplay} date at {dateInfo.time} on 2/
                {14 + dateInfo.day}.
              </Modal.Title>
            </Modal.Header>
            <Modal.Body>
    ...
    if (dateInfo.invited) {
        ...
        return (
          <div sx={datesSx}>
            <div className="date-banner blue-background" id="banner">
              We'll let you know when {otherName} accepts.
              <div
                onClick={() => this.toggleDateModal(true)}
                className="view-modal"
              >
      ...
      return (
        <div sx={datesSx}>
          <div className="date-banner blue-background" id="banner">
            <div>
              {otherName} invited you to a {dateOptionDisplay} date on 2/
          ...
          >
            ...
            <Modal.Body>
              <div className="description">
                <div>
                  {otherName} suggested a {dateOptionDisplay} date at{' '}
                  {dateInfo.time} on 2/{14 + dateInfo.day}.
                </div>
                {fullDateDescription}
                <br />
          ...
          <Modal.Body>
            ...
            <div>
              ...
              <div className="where-container">
                <Select
                  className="select background-border"
                  handleInputChange={this.handleInputChange}
                  labels={dateOptionNames}
                  name="dateOptionId"
                  placeholder="Options"
                  values={Object.keys(unsponsoredDateOptions)}
