...
class DormMatches extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  componentDidUpdate() {
    ...
    if (!schoolsList.includes(this.state.school)) {
      // default to Harvard
      this.setState({ school: convertCollege('Harvard') });
      this.createChord(this.state.data[convertCollege('Harvard')]);
    } else {
  ...
  createChord(selectedData) {
    ...
    var parentWidth = d3.select('#dorms-chord').node().getBoundingClientRect()
      .width;
    ...
    if (this.state.school === 'UW Madison') {
      outerRadius = width * 0.3 - 15;
      innerRadius = outerRadius - 10;
    }
    ...
    // ---create index array of dorm names: ["Canaday": 0, "Mather": 1, ..]---
    var namesArray = [];
    var houseIndex = [];
    var i = 0;
    for (let house in input.link_repeats) {
    ...
    d3.select('#dorms-chord').select('svg').remove();
    d3.select('#dorms-chord').select('.tooltip').remove();
...
export default firebaseConnect()(DormMatches);
